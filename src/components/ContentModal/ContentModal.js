...
export default function ContentModal({children, media_type, id}) {
  ...
  const fetchData = async () => {
    const {data} = await axios.get(
      `https://api.themoviedb.org/3/${media_type}/${id}?api_key=ea43363f811a71fcbfe8b52f2c68e898&language=en-US`
    );
    return data;
    // setContent(data);
  };
  const fetchVideo = async () => {
    const {data} = await axios.get(
      `https://api.themoviedb.org/3/${media_type}/${id}/videos?api_key=ea43363f811a71fcbfe8b52f2c68e898&language=en-US`
    );
    return data;
